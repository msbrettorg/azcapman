...
- When you need to file a quota increase from automation, submit the request with [`az quota request create`](https://learn.microsoft.com/en-us/cli/azure/quota?view=azure-cli-latest) and record the request ID so you can track approvals and retry deployments deterministically.
